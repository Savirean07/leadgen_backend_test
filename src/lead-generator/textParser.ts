interface ReadTempDataOptions {
  regex?: RegExp;
  duration?: number;
  callback: (data: { data: string | null; ended: boolean }) => void;
}
...
interface RegexExpression {
  ...
  AGENT: RegExp;
  NEXT_SPEAKER: RegExp;
  IGNORE_TERMINATE: RegExp;
  STAR_WITH_CHAR_CONTENT: RegExp;
  ONLY_STAR_CONTENT: RegExp;
}

const getLeadInformation = (text: string) => {
...
export class TransformData {
  agentData: AgentData;
  isSectionEnded: boolean;
  regexExpression: RegexExpression;
  ignoreContent: boolean;
  startPushContent: boolean;
  constructor() {
    this.agentData = {} as AgentData;
    ...
    this.regexExpression = {
      ...
      STAR_WITH_CHAR_CONTENT:
        /\*{4,} Response from calling function .*? \*{4,}/,
      ONLY_STAR_CONTENT: /^\*{4,}$/,
    };
  }

  readTempData(dataBlock: string, options: ReadTempDataOptions) {
  ...
  transformData(rawData: string | null, counter: number) {
    const {
      AGENT,
      FEEDBACK,
      ONLY_STAR_CONTENT,
    ...
    this.startPushContent = false;
    this.agentData = {} as AgentData;
    if (!rawData) return this.agentData;
    ...
    for (const line of lines) {
      ...
      if (NEXT_SPEAKER.test(line)) {
        this.agentData = {
          ...this.agentData,
          next_speaker: line.replace(NEXT_SPEAKER, "").trim(),
        };
        continue;
      }
      ...
      if (/{'searchParameters':/i.test(line)) {
        this.agentData = {
          ...this.agentData,
          response_type: "search_parameters",
          content: line,
        };
        continue;
      }
      ...
      if (/subject: /i.test(line)) {
        ...
        this.agentData = {
          ...this.agentData,
          response_type: isFinalEmail ? "final_email" : "email",
          requested_to: "bot",
          status: isFinalEmail ? "completed" : "in_progress",
          content: line,
        };
        continue;
      }
    ...
    const refinedData = { ...data, ...this.agentData }
